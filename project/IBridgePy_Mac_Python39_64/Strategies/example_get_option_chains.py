...
from IBridgePy.IbridgepyTools import filter_options_by_closest_strike, filter_options_by_closest_expiry
...
def handle_data(context, data):
    ...
    df_closest_expiry = filter_options_by_closest_expiry(ans, dt.datetime.now())
    print(df_closest_expiry.tail())

    # Filter the options which has a strike price closest to a target strike price.
    df_closest_strike = filter_options_by_closest_strike(df_closest_expiry, 450.0)
