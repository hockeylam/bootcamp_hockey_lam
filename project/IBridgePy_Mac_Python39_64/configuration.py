...
def run_me(fileName=None, userManualInput=None, userConfig=None):
    logging.debug(f'{__name__}::run_me')
    should_run_init_func = True
    ...
    while True:
        try:
            c.run(run_init=should_run_init_func)
            break
            ...
            if e.error_code in [509, 504, 502, 10141, ErrorCode.ERR_LOST_HEART_BEAT.code] and autoReconnectPremium:
                ...
                print('IBridgePy will sleep 10 seconds and reconnect again. Please restart TWS or IB Gateway if IBridgePy still cannot get connected after a few tries.')
                should_run_init_func = False
                time.sleep(10)
                userConfig.brokerClient.disconnectWrapper()

                # Create a new client and re-connect
            ...
            print(f'run_me:: err={err}')
            if autoReconnectPremium:
                print(f'IBridgePy will re-connect after 10 seconds. Please restart TWS or IB Gateway if IBridgePy still cannot get connected after a few tries.')
                time.sleep(10)
                userConfig.brokerClient.disconnectWrapper()
                ...
                print(f'IBridgePy version={c.trader.versionNumber}. If you need help, please send email to IBridgePy@gmail.com. Terminate IBridgePy because error={err} {traceback.print_exc()}')
                break
    c.terminate()
...
def _build_config(userConfig, userManualInput=None, fileName=None):
    ...
    place_combo_orders = trader.place_combo_orders
