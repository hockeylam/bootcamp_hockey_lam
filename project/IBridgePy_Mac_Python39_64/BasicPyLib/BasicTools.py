...
def str_to_epoch(dt_str):
    # The format must be "20240823 12:25:43 US/Eastern" because brokerapi.execution.Execution returned from IB server is in this format

    # Use pytz to load timezone data and parse the datetime string
    try:
        t = dt.datetime.strptime(dt_str, "%Y%m%d %H:%M:%S %Z")
    except ValueError:
        # Parse the datetime part without the timezone
        dt_part = dt.datetime.strptime(' '.join(dt_str.split(' ')[:-1]), "%Y%m%d %H:%M:%S")

        # Get the timezone from the remaining part of the string and apply it
        t = dt_part.replace(tzinfo=pytz.timezone(dt_str.split()[-1]))

        # Convert datetime to epoch seconds
        # epoch_seconds = t.timestamp()
