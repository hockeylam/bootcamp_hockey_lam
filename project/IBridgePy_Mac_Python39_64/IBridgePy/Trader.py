...
from IBridgePy.IbridgepyTools import special_match, create_order, check_same_security, from_symbol_to_security, \
    print_decisions, symbol, build_combo_order
...
class Trader(object):
    ...
    def get_contract_details(self, secType, symbol, field, currency='USD', exchange='', primaryExchange='', expiry='',
                             strike=0.0, right='', multiplier='', localSymbol='', tradingClass='', waitForFeedbackInSeconds=30):
        security = Security(secType=secType, symbol=symbol, currency=currency, exchange=exchange,
                            ...
                            multiplier=multiplier, localSymbol=localSymbol, tradingClass=tradingClass)
    ...
    def initialize_Function(self):
        ...
        # Move these to brokerClient::request
    ...
    def place_combo_orders(self, securities, amount, comboDirection, style=MarketOrder(), outsideRth=False, accountCode='default', waitForFeedbackInSeconds=30, followUp=True):
        self._log.debug(f"{__name__}::place_combo_orders: securities={securities} amount={amount} comboDirection={comboDirection} style={style}")
        adj_accountCode = self.adjust_accountCode(accountCode)
        int_orderId = self._brokerService.use_next_id()
        ibridgePyOrder = build_combo_order(int_orderId, adj_accountCode, securities, abs(amount), comboDirection, style, outsideRth, self.get_datetime())
        return self._brokerService.place_order(ibridgePyOrder, followUp=followUp, waitForFeedbackInSeconds=waitForFeedbackInSeconds)
    ...
    def order_target(self, security, amount, style=MarketOrder(), orderRef='', followUp=True,
        ...
        self._log.debug(f'order_target: security={security.full_print()} amount={amount} current_hold={hold}')
        if amount != hold:
    ...
    def _build_order_helper(self, security, amount, style=MarketOrder(), orderRef='', accountCode='default'):
        ...
        parentOrder = create_order(int_parentOrderId, adj_accountCode, security, amount, style, self.get_datetime(), orderRef=orderRef)
        int_slOrderId = self._brokerService.use_next_id()
        return adj_accountCode, parentOrder, int_slOrderId, int_parentOrderId

    def place_order_with_stoploss(self, security, amount, stopLossPrice, style=MarketOrder(), tif=None, orderRef='',
                                  accountCode='default'):
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
        adj_accountCode, parentOrder, int_slOrderId, int_parentOrderId = self._build_order_helper(security, amount, style, orderRef, accountCode)
        slOrder = create_order(int_slOrderId, adj_accountCode, security, -amount, StopOrder(stopLossPrice, tif=tif), self.get_datetime(), orderRef=orderRef)
    ...
    def place_order_with_takeprofit(self, security, amount, takeProfitPrice, style=MarketOrder(), tif=None, orderRef='', accountCode='default'):
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
        adj_accountCode, parentOrder, int_tpOrderId, int_parentOrderId = self._build_order_helper(security, amount, style, orderRef, accountCode)
        tpOrder = create_order(int_tpOrderId, adj_accountCode, security, -amount, LimitOrder(takeProfitPrice, tif=tif), self.get_datetime(), orderRef=orderRef)
        # IB recommends this way to place takeProfitOrder and stopLossOrder
        # with main order.
        parentOrder.requestedOrder['transmit'] = False
    ...
    def place_order_with_stoploss_takeprofit(self, security, amount, stopLossPrice, takeProfitPrice, style=MarketOrder(tif=None), tif=None, accountCode='default', orderRef=''):
        """
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
    ...
    def get_all_orders(self, accountCode='default'):
        """
        For Web-api-based brokers, it returns all orders that are returned from the broker.
    ...
    def _check_schedules(self):
        ...
        for ct in self.scheduledFunctionList:
            if special_match(ct.onHour, timeNow.hour, 'hourMinute') and \
                    ...
                    special_match(ct.onSecond, timeNow.second, 'hourMinute'):
        ...
        if date_rule is None:
            ...
            if date_rule.version == 'every_day':
                ...
                if date_rule.version == 'week_start':
                    raise NotImplementedError('date_rule.week_start')
                elif date_rule.version == 'week_end':
                    raise NotImplementedError('date_rule.week_end')
                if date_rule.version == 'month_start':
                    raise NotImplementedError('date_rule.month_start')
                elif date_rule.version == 'month_end':
                    raise NotImplementedError('date_rule.month_end')
