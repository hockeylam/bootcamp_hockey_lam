...
def from_contract_to_security(contract):
    ...
    for para in ['secType', 'symbol', 'primaryExchange', 'exchange', 'currency', 'lastTradeDateOrContractMonth', 'strike', 'right', 'multiplier', 'localSymbol', 'tradingClass']:
        tmp = contract.get(para, '')
        if tmp:
            # IB has moved from expiry to lastTradeDateOrContractMonth in the latest versions
            # But, symbol in IBridgePy is still using expiry.
            # Therefore, the value of lastTradeDateOrContractMonth has to been mapped to expiry.
            if para == 'lastTradeDateOrContractMonth':
                setattr(ans, 'expiry', tmp)
            else:
...
def from_security_to_contract(security):
    contract = {'symbol': security.symbol,
                ...
                'tradingClass': security.tradingClass,
...
class Security(object):
    def __init__(self,
        ...
        self.comboLegDirection = comboLegDirection
        self.comboLegRatio = comboLegRatio
    ...
    def full_print(self):  # VERY IMPORTANT because context.portfolio.positions using security.full_print as the key.
        ...
        # because strike is not the same.
            ...
            return '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s' % (self.secType, self.primaryExchange, self.exchange, self.symbol, self.currency,
                                                      self.expiry, "{:10.2f}".format(self.strike), self.right, self.multiplier, self.tradingClass)
