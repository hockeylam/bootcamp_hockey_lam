...
class Converter(object):
    ...
    def from_orderId_int_to_orderId_str(self, orderId):
        return 'ib' + str(orderId)

    def from_orderId_string_to_orderId_int(self, orderId):
        # print(f'from_orderId_string_to_orderId_int: orderId={orderId}')
        return int(orderId[2:])
    ...
    def fromBrokerToIB(self, brokerValue):
        ...
        if self._brokerClientName in [BrokerClientName.IB, BrokerClientName.LOCAL, BrokerClientName.IBinsync]:
            ...
            ibValue = self._createrOfIBValue.use_next_id()
        else:
