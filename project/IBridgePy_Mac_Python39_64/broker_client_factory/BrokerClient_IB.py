...
def _parse_exchange(txt):
    x = re.search("{.*}", txt)
    if x:
        return list(txt[x.span()[0]: x.span()[1]])
    else:
        return txt
...
class ClientIB(CallBacks):
    ...
    def connectWrapper(self):
        ...
        if not self.isConnected():
            self._log.debug(f'{__name__}::connectWrapper: host={self.host} port={self.port} clientId={self.clientId}')
            self._log.info(f'Try to get connected to Interactive Brokers')
            self.connect(self.host, int(self.port), int(self.clientId))

        if self.isConnected():
            time.sleep(2)  # It is critical to sleep 1 second after get connected because it take a little bit time for IB Gateway to update its status.
    ...
    def reqHeartBeatsWrapper(self):
        self.req_heart_beats()  # IBCpp function
    ...
    def cancelOrderWrapper(self, ibpyOrderId):
        ibOrderId = self._idConverter.from_orderId_string_to_orderId_int(ibpyOrderId)
    ...
    def add_primaryExchange_to_security(self, security):
        ...
        if security.secType == 'CASH':
            ...
            return
        if security.exchange != 'SMART':
            ans = self.get_contract_details(security, 'primaryExchange')['primaryExchange']
            # !!! for META, the return are as follows so that it cannot be used directly without parsing
            # {'primaryExchange': "many contracts (27) have different primaryExchange. The values are {'NASDAQ', 'LSEETF'}",
            #  'exchange': "many contracts (27) have different exchange. The values are {'PHLX', 'BYX', 'NYSENAT', 'DRCTEDGE', 'EDGEA'}"}
            if 'values are' in ans:
                ans = _parse_exchange(ans)
            self._log.debug(__name__ + '::add_primaryExchange_to_security: Found from IB server ans=%s' % (ans,))
            if isinstance(ans, list):
                security.primaryExchange = ans[0]
            else:
