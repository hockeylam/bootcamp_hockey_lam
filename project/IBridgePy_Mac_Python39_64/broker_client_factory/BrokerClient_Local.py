...
class ClientLocalBroker(CallBacks):
    ...
    def cancelOrderWrapper(self, ibpyOrderId):
        ...
        for order_id in orders_to_update:
            ...
            self._orderToBeProcessed._purely_delete_order_ignoring_ocaGroup(order_id)
    ...
    def simulate_process_order(self, timeNow):  # to simulate placing order
        ...
        for int_orderId in active_orderId_list:
            ...
            if Decimal(order['totalQuantity']) == Decimal(0):
                self.simulateOrderStatus(int_orderId, 'Filled', str(order['totalQuantity']), '0', 0.0, 0, 0, 0.0, 0, 'empty', 0.0)  # IBCpp function
            ...
            if flag and ex_price > 0.001:
                self.simulateOrderStatus(int_orderId, 'Filled', str(order['totalQuantity']), '0', ex_price, 0, 0, ex_price, 0, 'empty', 0.0)  # IBCpp function
                self._orderToBeProcessed.fill_order(int_orderId)
                ...
                if order.get('parentId', None) not in [0, None] and orders_to_update is not None:
                    for order_id in orders_to_update:
                        if order_id != int_orderId:
                            self.simulateOrderStatus(order_id, OrderStatus.CANCELLED, '0', '0', 0.0, 0, 0, 0.0, 0, 'empty', 0.0)  # IBCpp function
                # The hist data provide by the users can be any format after parsing by numpy. For example, 1234 will be parsed as numpy.int64. However, Decimal cannot convert from numpy.int64 to Decimal, which leads to some errors.
                # Therefore, the solution is to force the type of ex_price to float
                ex_price = float(ex_price)
                ...
                execution['time'] = dt_to_epoch(timeNow)
                if order['action'] == 'BUY':
                    execution['side'] = 'BOT'
                else:
                    execution['side'] = 'SLD'
