...
def _build_IB_client_search_port_get_connected(userConfig, searchPort, portList, newTrader, newDataFromServer, ibInsync=False):
    # print(f'_build_IB_client_search_port_get_connected: userConfig searchPort={searchPort} portList={portList} ibInsync={ibInsync}')
    t = None
    if newTrader:
    ...
    # For user's convenience, IBridgePy searches default hosts.
    hosts = ['localhost', '127.0.0.1']
    if userConfig.brokerClientConfig.IB_CLIENT['host']:
        if userConfig.brokerClientConfig.IB_CLIENT['host'] not in hosts:
            # If user specifies a host, add it to the first place in hosts.
            hosts = [userConfig.brokerClientConfig.IB_CLIENT['host']] + hosts
        else:
            hosts.remove(userConfig.brokerClientConfig.IB_CLIENT['host'])
            hosts = [userConfig.brokerClientConfig.IB_CLIENT['host']] + hosts
    if userConfig.brokerClientConfig.IB_CLIENT['port']:
        if userConfig.brokerClientConfig.IB_CLIENT['port'] not in portList:
            # If user specifies a port, add it to the first place in portList.
            portList = [userConfig.brokerClientConfig.IB_CLIENT['port']] + portList
        else:
            # If user provides a port and the port exists, sort so that the port provided by the user is the first one.
            portList.remove(userConfig.brokerClientConfig.IB_CLIENT['port'])
            portList = [userConfig.brokerClientConfig.IB_CLIENT['port']] + portList
...
class BrokerClientFactory(object):
    ...
    def _create_brokerClient_by_name(self, brokerClientName, userConfig):
        ...
        if brokerClientName == BrokerClientName.LOCAL:
            ...
            t = _build_IB_client_search_port_get_connected(userConfig, True, userConfig.projectConfig.searchPortList, True, True, ibInsync=False)
            t.connectWrapper()
        elif brokerClientName == BrokerClientName.IBinsync:
            t = _build_IB_client_search_port_get_connected(userConfig, True, userConfig.projectConfig.searchPortList, True, True, ibInsync=True)
    ...
    def get_brokerClient_by_userConfig(self, userConfig, check_cache=True, delete_cache=False):
        ...
        if brokerClientName == BrokerClientName.LOCAL:
            ...
            t = _build_IB_client_search_port_get_connected(userConfig, True, userConfig.projectConfig.searchPortList, False, False)

        elif brokerClientName == BrokerClientName.IBinsync:
            t = _build_IB_client_search_port_get_connected(userConfig, True, userConfig.projectConfig.searchPortList, False, False, True)
