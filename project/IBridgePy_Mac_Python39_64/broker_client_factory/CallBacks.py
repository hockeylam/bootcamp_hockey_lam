...
from BasicPyLib.BasicTools import epoch_to_dt, dt_to_epoch, str_to_epoch
...
def from_execution_to_dict(execution, show_timezone=None):
    ...
    # print(f'from_execution_to_dict: execution={execution}')
    if isinstance(execution['time'], int):
        execution['time'] = epoch_to_dt(execution['time'])
    else:
        execution['time'] = epoch_to_dt(str_to_epoch(execution['time']))

    if show_timezone:
        execution['time'] = execution['time'].astimezone(show_timezone)
    return execution
...
class CallBacks(BrokerClientBase):
    ...
    def error(self, errorId, errorCode, errorMessage, advancedOrderRejectJson):
        ...
        if errorCode in [509, 10141]:
        ...
        elif errorCode in [201, 399, 2113, 2148, 2109, 10311]:  # No action, just show error message
            ...
            # 10311: This order will be directly routed to CBOE. Direct routed orders may result in higher trade fees. Restriction is specified in Precautionary Settings of Global Configuration/API.
            # 399: Order Message: BUY 100 SPY ARCA Your order will not be placed at the exchange until 2024-03-18 09:30:00 US/Eastern. If you want the order to be active in the Pre-Market, Regular and After-Market sessions, a Limit type order is required and you must add the attribute "Fill/Trigger Outside RTH." Click  <a href="https://www.ibkrguides.com/kb/article-576.htm?cid=d6326009-23da-46a1-8f9b-9b3f9f9087ca"> here </a> for more information regarding stocks Outside of the Regular Trading Hours sessions. advancedOrderRejectJson=
            self._log.error(
            ...
            self._log.error(__name__ + f'::error: errorId={errorId} errorCode={errorCode} errorMessage={errorMessage} advancedOrderRejectJson={advancedOrderRejectJson}')
            self._log.error('Hint: Please refer to YouTube tutorial https://youtu.be/pson8T5ZaRw')
            self.e_disconnect()
            self._print_version_and_exit()
    ...
    def _print_version(self):
        self._log.error('IBridgePy version= %s' % (str(self.versionNumber),))

    def _print_version_and_exit(self):
        self._log.error('EXIT IBridgePy version= %s' % (str(self.versionNumber),))
        exit()
    ...
    def execDetails(self, reqId, contract, execution):
        ...
        :param reqId:
        :param contract: brokerapi.contract.Contract
        :param execution: brokerapi.execution.Execution
        :return:
        """
        contract = from_contract_to_dict(contract)
        execution = from_execution_to_dict(execution, show_timezone=self._showTimeZone)
        self._log.debug(f'{__name__}::execDetails: reqId={reqId} contract={print_IBCpp_contract(contract)} execution={print_IBCpp_execution(execution)}')
    ...
    def nextValidId(self, nextId):
        ...
        self._nextId.set_next_id(nextId)
    ...
    def openOrder(self, int_ibOrderId, contract, order, orderState):
        ...
        self._log.debug(f'{__name__}::openOrder: ibOrderId={int_ibOrderId} contract={contract} order={order} orderState={orderState}')
    ...
    def openOrderEnd(self):
        ...
        if self._activeRequests:  # After the initial connection, IBKR may callback a few things and IBridgePy has no idea about them.
    ...
    def orderStatus(self, int_orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice,
        ...
        # Then, create a string value for it using permId
        # IBridgePy will not touch these orders and the created value is not registered in idConverter
        if int_orderId in [-1, 0]:
            str_ibpyOrderId = f'permIDatIB{permId}'
        else:
    ...
    def tickOptionComputation(self, reqId, tickType, tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma, vega, theta, undPrice):
        self._log.debug(f'{__name__}::tickOptionComputation: reqId={reqId} tickType={tickType} tickAttrib={tickAttrib} impliedVol={impliedVol} delta={delta} optPrice={optPrice} pvDividend={pvDividend} gamma={gamma} vega={vega} theta={theta} undPrice={undPrice}')
        ...
        if self._activeRequests.is_reqId_within_activeRequests(reqId):
            aRequest = self._activeRequests.get_by_reqId_otherwise_exit(reqId)
            if aRequest.follow_up_on != 'tickOptionComputation':
                self._log.debug(f"{__name__}::tickOptionComputation: Ignore this record because aRequest.follow_up_on={aRequest.follow_up_on}")
                return
        ...
        str_security = self._allRequests[reqId].param['security'].full_print()
        # For any reason, the reqId is not in the self.activeRequests, just update the price, not need to be an active request at all.
        self._dataFromServer.set_tickInfoRecord(TickOptionComputationRecord(str_security, tickType, tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma, vega, theta, undPrice))
    ...
    def tickPrice(self, reqId, tickType, price, canAutoExecute):
        ...
        """
        timestamp = self._timeGenerator.get_current_time()
        self._log.notset(f"{__name__}::tickPrice: reqId={reqId} tickType={tickType} price={price} canAutoExecute={canAutoExecute} timestamp={timestamp}")
        ...
        if self._activeRequests.is_reqId_within_activeRequests(reqId):
            aRequest = self._activeRequests.get_by_reqId_otherwise_exit(reqId)
            if aRequest.follow_up_on != 'tickPrice':
                self._log.debug(f"{__name__}::tickPrice: Ignore this record because aRequest.follow_up_on={aRequest.follow_up_on} reqId={reqId} tickType={tickType} price={price} canAutoExecute={canAutoExecute} timestamp={timestamp}")
                return
            if aRequest.status != ReqAttr.Status.COMPLETED:
                # https://interactivebrokers.github.io/tws-api/tick_types.html
                try:
                    if tickType == 1 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.BID:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 2 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.ASK:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 4 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.LAST:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 6 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.HIGH:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 7 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.LOW:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 9 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.CLOSE:
                        aRequest.status = ReqAttr.Status.COMPLETED
                    elif tickType == 14 and aRequest.param['tickTypeClientAsked'] == IBCpp.TickType.OPEN:
                        aRequest.status = ReqAttr.Status.COMPLETED
                except KeyError as error:
                    aRequest.status = ReqAttr.Status.COMPLETED
                    self._log.info(f"tickPrice received an unrecognized reqeust={aRequest} error={error}. Please send this message to IBridgePy.com for further analysis.")
    ...
    def tickSize(self, reqId, tickType, size):
        ...
        self._log.notset(f'{__name__}::tickSize: reqId={reqId} tickType={tickType} size={size}')
