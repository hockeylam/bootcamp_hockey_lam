...
from IBridgePy.Trader import TICK_TYPE_MAPPER
...
class BrokerService(object):
    ...
    def _get_account_info_one_tag(self, accountCode, tag, meta='value'):
        NotImplementedError(f'NotImplementedError: {__name__}')
    ...
    def get_option_info(self, security, fields, waitForFeedbackInSeconds=30):
        ...
        if not self._brokerClient.is_real_time_price_requested(security):
            self.submit_requests(ReqMktData(security, waitForFeedbackInSeconds=waitForFeedbackInSeconds, tickTypeClientAsked=TICK_TYPE_MAPPER.get('last_price'), follow_up_on='tickOptionComputation'))
        ans = {}
        for fieldName in fields:
            if fieldName in ['delta', 'gamma', 'vega', 'theta', 'impliedVol', 'optPrice', 'undPrice']:
                ans[fieldName] = self._dataFromServer.get_value(security, 13, fieldName)  # https://interactivebrokers.github.io/tws-api/tick_types.html 13 = Model Option Computation. Less issues when keeping using 13
            elif fieldName in ['option_call_open_interest']:
                ans[fieldName] = self._dataFromServer.get_value(security, 27, 'size')  # 27 = IBCpp.TickType.OPTION_CALL_OPEN_INTEREST. Less issues when keeping using 27
            elif fieldName in ['option_put_open_interest']:
                ans[fieldName] = self._dataFromServer.get_value(security, 28, 'size')  # 28 = IBCpp.TickType.OPTION_PUT_OPEN_INTEREST. Less issues when keeping using 28
            else:
