...
from brokerapi.message import OUT
from brokerapi.common import * # @UnusedWildImport
from brokerapi.contract import Contract
from brokerapi.order import Order, COMPETE_AGAINST_BEST_OFFSET_UP_TO_MID
...
from brokerapi.utils import (current_fn_name, BadMessage)
from brokerapi.errors import * #@UnusedWildImport
from brokerapi.server_versions import * # @UnusedWildImport
from brokerapi.utils import ClientException
from brokerapi.converter import order_converter, contract_converter, scannerSubscription_converter
...
class EClient(object):
    ...
    def __init__(self, wrapper):
        ...
        self.decoder = None
        self.reset(f'{__name__}.__init__')

    def reset(self, caller=None):
        logging.debug(f'{__name__}::reset: caller={caller}')
        self.nKeybIntHard = 0
        self.conn = None
    ...
    def connect(self, host, port, clientId):
        ...
        try:
            ...
            count = 0
            COUNT_THRESHOLD = 1
            while len(fields) != 2 and count <= COUNT_THRESHOLD:
                count += 1
    ...
    def reqMktData(self, reqId:TickerId, contract:Contract,
        ...
        contract = contract_converter(contract)
        mktDataOptions = str(mktDataOptions)
    ...
    def reqScannerSubscription(self, reqId:int,
        ...
        scannerSubscriptionOptions:TagValueList - For internal use only.
            Use default value XYZ."""
        subscription = scannerSubscription_converter(subscription)
        self.logRequest(current_fn_name(), vars())
