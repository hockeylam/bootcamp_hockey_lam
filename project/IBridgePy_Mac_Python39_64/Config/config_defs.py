...
from IBridgePy.IbridgepyTools import read_and_parse_identity_file
...
# noinspection PyPep8Naming
class ENVIRONMENT_Config(Config):
    def __init__(self, dict_settings):
        Config.__init__(self, dict_settings)
...
class UserConfigBase(Printable):  # should not be used directly
        ...
        for item in dir(module_settings):
            if isAllLettersCapital(item):
                if item == 'PROJECT':
                    ...
                    self.environmentConfig = ENVIRONMENT_Config(getattr(module_settings, item))
                else:
        ...
        for item in dir(module_settings):
            if isAllLettersCapital(item):
                if item == 'PROJECT':
                    ...
                    self.environmentConfig.override(getattr(module_settings, item), str_settings)
                else:
    ...
    def overrideByConfigDict(self, dict_passedIn):
        ...
        for item in dict_passedIn:
            ...
            if item == 'PROJECT':
                ...
                self.environmentConfig.override(dict_passedIn[item], fileName)
            else:
                continue
        ...
        if self.projectConfig.liveOrBacktest == LiveBacktest.BACKTEST:
            ...
            if self.backtesterConfig.overrideValues is True:
                if self.traderConfig.runMode != TraderRunMode.RUN_LIKE_QUANTOPIAN:
                    ...
                    # if self.timeGeneratorConfig.timeGeneratorType != TimeGeneratorType.CUSTOM:
                    #     self.traderConfig.runMode = TraderRunMode.RUN_LIKE_QUANTOPIAN
                    #     print('IBridgePy has reset the TRADER.runMode to RUN_LIKE_QUANTOPIAN for backtesting to save time!')
    ...
    def load_userInput_to_userConfig(self, user_input):
        ...
        if hasattr(user_input, 'overrideValues'):
            self.backtesterConfig.set_value('overrideValues', user_input.overrideValues, __name__ + '::load_userInput_to_userConfig')
    ...
    def _load_identity_to_userConfig(self):
        identity = read_and_parse_identity_file()
        self.environmentConfig.sys = str(identity.get('sys')).strip()
        self.environmentConfig.python_type = str(identity.get('python')).strip()
        self.environmentConfig.python_version = str(identity.get('version')).strip()
        self.environmentConfig.bit = str(identity.get('bit')).strip()
        self.environmentConfig.in_docker = str(identity.get('docker', 'False').strip())
        if self.environmentConfig.in_docker.lower() in ['true']:
            self.brokerClientConfig.IB_CLIENT['host'] = 'host.docker.internal'
    ...
    def prepare_userConfig_with_trader(self, trader, globalsV):
        ...
        # environment config comes from identity.csv
        self._load_identity_to_userConfig()
